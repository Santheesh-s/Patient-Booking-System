...
export const initializeDatabase: RequestHandler = async (req, res) => {
  try {
    ...
    const { _id: _a, ...adminInsert } = adminUser;
    await db.collection("users").insertOne(adminInsert as any);
    ...
    const { _id: _p, ...providerInsert } = provider;
    const providerResult = await db.collection("providers").insertOne(providerInsert as any);
    ...
    const { _id: _pu, ...providerUserInsert } = providerUser;
    await db.collection("users").insertOne(providerUserInsert as any);
    ...
    const servicesForInsert = services.map(({ _id, ...s }) => s);
    const servicesResult = await db.collection("services").insertMany(servicesForInsert as any);
    ...
    const { _id: _av, ...availabilityInsert } = availability;
    await db.collection("availability").insertOne(availabilityInsert as any);
