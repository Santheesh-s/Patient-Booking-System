...
import ProviderServicesSelfAssign from "@/components/Provider/ProviderServicesSelfAssign";
import ProviderAvailabilitySelf from "@/components/Provider/ProviderAvailabilitySelf";
...
export default function AdminDashboard() {
  ...
  const [showSelfServicesModal, setShowSelfServicesModal] = useState(false);
  const [showSelfAvailabilityModal, setShowSelfAvailabilityModal] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {showSelfServicesModal && user?.role === "provider" && (
          <ProviderServicesSelfAssign
            providerId={user.providerId}
            onClose={() => setShowSelfServicesModal(false)}
          />
        )}
        {showSelfAvailabilityModal && user?.role === "provider" && (
          <ProviderAvailabilitySelf
            providerId={user.providerId}
            onClose={() => setShowSelfAvailabilityModal(false)}
          />
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mt-8">
          ...
          {user?.role === "provider" && (
            <>
              <Card className="border-2 cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition">
                ...
                <CardContent className="text-center py-6 sm:py-8">
                  <p className="text-xs sm:text-sm text-muted-foreground mb-4">
                    Adjust your working hours & blocked dates
                  </p>
                  <Button
                    onClick={() => setShowSelfAvailabilityModal(true)}
                    className="bg-primary hover:bg-primary/90 text-white"
                  >
                    Manage Availability
                  </Button>
              ...
              <Card className="border-2 hover:border-primary/50 hover:bg-primary/5 transition">
                <CardHeader>
                  <CardTitle className="text-base sm:text-lg flex items-center gap-2">
                    <Settings className="w-5 h-5" />
                    My Services
                  </CardTitle>
                  <CardDescription>View or map services to your profile</CardDescription>
                </CardHeader>
                <CardContent className="text-center py-6 sm:py-8 space-y-3">
                  <p className="text-xs sm:text-sm text-muted-foreground">
                    These are the services linked to your provider account.
                  </p>
                  <div className="flex gap-2 justify-center">
                    <Button
                      onClick={() => setShowSelfServicesModal(true)}
                      className="bg-primary hover:bg-primary/90 text-white"
                    >
                      Manage My Services
                    </Button>
