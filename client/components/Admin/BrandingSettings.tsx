...
function applyBrandingToDocument(s: BrandingSettings) {
...
export default function BrandingSettings({ onClose }: BrandingSettingsProps) {
	...
	const [activeTab, setActiveTab] = useState<"branding" | "notifications" | "business">(
		"branding"
	);
	...
	const fetchSettings = async () => {
		...
		try {
			...
			if (!response.ok) {
				setSettingsError(`Failed to load settings (${response.status})`);
				setSettings(null);
				return;
			}
	...
	const handleSaveSettings = async () => {
		...
		try {
			...
			setSettings(updated);
			applyBrandingToDocument(updated); // apply immediately
			toast({
				title: "Success",
	...
	const handleResetSettings = async () => {
		...
		try {
			...
			if (response.ok) {
				const data = await response.json();
				setSettings(data);
				applyBrandingToDocument(data); // apply after reset
				toast({
					title: "Success",
			...
			<Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto border-2">
				<CardHeader className="sticky top-0 bg-background border-b flex flex-row items-center justify-between">
					<div>
						<CardTitle>Clinic Settings</CardTitle>
						<CardDescription>
							Customize your clinic branding and preferences
						</CardDescription>
					</div>
					<Button
						variant="ghost"
						...
						<div className="space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div>
									...
									<input
										...
										onChange={(e) =>
											updateSetting("clinicWebsite", e.target.value)
										}
										className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"
									/>
								</div>
								<div>
									...
									<input
										...
										onChange={(e) =>
											updateSetting("clinicEmail", e.target.value)
										}
										className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"
									/>
								</div>
								<div>
									...
									<input
										...
										onChange={(e) =>
											updateSetting("clinicPhone", e.target.value)
										}
										className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"
									/>
								</div>
							...
							<div>
								...
								<textarea
									...
									onChange={(e) =>
										updateSetting("clinicAddress", e.target.value)
									}
									rows={3}
									className="w-full mt-1 px-3 py-2 border border-border rounded-lg bg-background"
								/>
								...
								<div className="ml-7 space-y-3 border-l-2 border-border pl-4">
									...
									<label className="flex items-center gap-3 cursor-pointer">
										<input
											...
											onChange={(e) =>
												updateSetting("smsNotificationsEnabled", e.target.checked)
											}
											disabled={!settings.notificationsEnabled}
											className="w-4 h-4 rounded border border-border"
										/>
							...
							<div className="border-t border-border pt-6">
								<label className="flex items-center gap-3 cursor-pointer">
									<input
										...
										onChange={(e) =>
											updateSetting("bookingApprovalRequired", e.target.checked)
										}
										className="w-4 h-4 rounded border border-border"
									/>
									<div>
					...
					{activeTab === "business" && (
						<div className="space-y-6">
							...
							<div className="bg-blue-50 border border-blue-200 rounded p-3 text-sm text-blue-900">
								<strong>💡 Note:</strong> These business hours are used for
								default availability calculations. Individual providers can have
								custom hours.
							</div>
						</div>
					)}
