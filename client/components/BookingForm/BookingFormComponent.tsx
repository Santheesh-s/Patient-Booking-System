...
export default function BookingFormComponent({ onHome }: BookingFormProps) {
  ...
  const fetchSlots = async (providerId: string, date: string, duration: number) => {
    ...
    try {
      ...
      // Remove any stale past slots (client-side safeguard)
      const now = Date.now();
      setSlots(data.filter(s => new Date(s.startTime).getTime() > now));
    } catch (error) {
